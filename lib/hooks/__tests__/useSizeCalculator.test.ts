...
const getExpectedOffset = (eventStartDate: Date, eventEndDate: Date): string => {
  const timelineDatesDiff: number = Math.abs(differenceInMilliseconds(TODAY_END, TODAY_START));
  const eventDatesDiff: number = Math.abs(differenceInMilliseconds(eventEndDate, eventStartDate));
