...
const timeEvents = [
  { id: 1, label: 'Daily call', date: add(now, { hours: 1 }) },
  { id: 2, label: 'Deadline', date: add(now, { hours: 4, minutes: 30 }) },
];
...
function App() {
  return (
    <div style={{ position: 'relative', width: '100%', maxWidth: '80vw' }}>
      <Timeline.Provider startDate={timeline.startDate} endDate={timeline.endDate}>
        <>
          ...
          >
            ...
            {timeEvents.map(({ id, label, date }) => (
              <TimeIndicator key={id} label={label} date={date} />
            ))}

            <Timeline.Indicators.CurrentTime
              render={({ currentTime, styles }) => <CurrentTime currentTime={currentTime} style={styles} />}
            />
          </div>
